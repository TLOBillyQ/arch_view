...
local tmp_root = common.join_path(common.system_tmp_dir(), "arch_view_test_go_bridge")
...
local function test_go_bridge_analyze()
    _with_clean_tmp(function()
        local project_root = common.join_path(tmp_root, "bridge_analyze")
        _write_sample_project(project_root)

        local architecture, binary_path = go_bridge.analyze({
...
local function test_go_bridge_check()
    _with_clean_tmp(function()
        local project_root = common.join_path(tmp_root, "bridge_check")
        _write_sample_project(project_root)

        local check_result, binary_path = go_bridge.check({
            project_root = project_root,
            config_path = common.join_path(project_root, "arch_view.config.json"),
            config = {
                source_roots = {"src"},
...
local function test_go_bridge_export_viewer()
    _with_clean_tmp(function()
        ...
        local result, binary_path = go_bridge.export_viewer({
            project_root = project_root,
            config_path = common.join_path(project_root, "arch_view.config.json"),
            config = {
                source_roots = {"src"},
...
return {
    test_go_bridge_ensure_binary = test_go_bridge_ensure_binary,
    test_go_bridge_analyze = test_go_bridge_analyze,
    test_go_bridge_check = test_go_bridge_check,
    test_go_bridge_write_architecture_json = test_go_bridge_write_architecture_json,
    test_go_bridge_export_viewer = test_go_bridge_export_viewer,
}
