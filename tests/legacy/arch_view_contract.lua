...
local function _write_sample_project(project_root)
    ...
    _write_file(common.join_path(project_root, "src/demo/pkg/init.lua"),
        'local beta = require("src.demo.beta")\nreturn beta\n')
...
local function _test_build_and_cli_respect_default_config_path()
    _with_clean_tmp(function()
        ...
        _assert_eq(architecture.modules["src.demo.pkg"].component, "demo",
            "build should classify modules from external config")
...
local function _test_public_api_uses_default_config_and_exports_viewer()
    _with_clean_tmp(function()
        ...
        assert(_exists(common.join_path(project_root, ".arch_view/viewer/index.html")),
            "export_viewer should use default output directory")
        assert(_exists(common.join_path(project_root, ".arch_view/viewer/architecture_data.js")),
            "export_viewer should write viewer payload")
    end)
end
...
local function _test_viewer_export_is_self_contained_and_generic()
    _with_clean_tmp(function()
        ...
        _assert_not_contains(exported_styles, "\"Instrument Serif\"",
            "viewer styles should not reference external font families")
    end)
end

local function _test_go_engine_matches_auto_engine_output()
    _with_clean_tmp(function()
        ...
        _assert_eq(json_writer.encode(normalized_go.classified_edges),
            json_writer.encode(normalized_auto.classified_edges),
            "go classified_edges should match auto classified_edges")
        _assert_eq(json_writer.encode(normalized_go.projection_cycles),
            json_writer.encode(normalized_auto.projection_cycles),
            "go projection_cycles should match auto projection_cycles")
        _assert_eq(json_writer.encode(normalized_go.check), json_writer.encode(normalized_auto.check),
            "go check should match auto check")
        _assert_eq(json_writer.encode(common.sorted_keys(normalized_go.views or {})),
            json_writer.encode(common.sorted_keys(normalized_auto.views or {})),
            "go view keys should match auto view keys")

        for _, view_key in ipairs(common.sorted_keys(normalized_auto.views or {})) do
            _assert_semantic_eq(
                _view_signature(normalized_go.views[view_key]),
                _view_signature(normalized_auto.views[view_key]),
                "go view should match auto view: " .. tostring(view_key)
            )
        end
    end)
end
...
return {
    tests = {
        ...
        { name = "go_engine_matches_auto_engine_output",              run = _test_go_engine_matches_auto_engine_output },
